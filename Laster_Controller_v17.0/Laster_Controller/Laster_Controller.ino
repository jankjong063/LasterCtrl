...
// Log details of cuts performed
void logCutDetails(String patternName, int duration) {
  File logFile = SD.open("cutLog.txt", FILE_WRITE);
  if (logFile) {
    logFile.print("Pattern: ");
    logFile.print(patternName);
...
void loop() {
  ...
  // Detect user activity (button press) to reset inactivity timer
  if (digitalRead(BUTTON_X_PLUS) == LOW || digitalRead(BUTTON_X_MINUS) == LOW ||
      digitalRead(BUTTON_Y_PLUS) == LOW || digitalRead(BUTTON_Y_MINUS) == LOW ||
      digitalRead(BUTTON_Z_PLUS) == LOW || digitalRead(BUTTON_Z_MINUS) == LOW) {
