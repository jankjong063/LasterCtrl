...
// Function to check for inactivity and turn off the laser if needed
...
void loop() {
  ...
  // Check button states and move motors accordingly, with debouncing
  if (!debounceButton(BUTTON_X_PLUS)) {
    moveStepper(X_STEP_PIN, X_DIR_PIN, 1);
    updateLCD("X", 1);
    updateActivity();  // Reset inactivity timer
  }
  if (!debounceButton(BUTTON_X_MINUS)) {
    moveStepper(X_STEP_PIN, X_DIR_PIN, -1);
    updateLCD("X", -1);
    updateActivity();
  }
  if (!debounceButton(BUTTON_Y_PLUS)) {
    moveStepper(Y_STEP_PIN, Y_DIR_PIN, 1);
    updateLCD("Y", 1);
    updateActivity();
  }
  if (!debounceButton(BUTTON_Y_MINUS)) {
    moveStepper(Y_STEP_PIN, Y_DIR_PIN, -1);
    updateLCD("Y", -1);
    updateActivity();
  }
  if (!debounceButton(BUTTON_Z_PLUS)) {
    moveStepper(Z_STEP_PIN, Z_DIR_PIN, 1);
    updateLCD("Z", 1);
    updateActivity();
  }
  if (!debounceButton(BUTTON_Z_MINUS)) {
    moveStepper(Z_STEP_PIN, Z_DIR_PIN, -1);
    updateLCD("Z", -1);
    updateActivity();
  }

  // Control laser on/off with a simple condition (for example)
  ...
  delay(100);  // General debounce delay
}
